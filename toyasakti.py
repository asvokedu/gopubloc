...
class SignalDetector:
    ...
    def __init__(self):
        ...
        self.AUTO_CLOSE_THRESHOLD_LOSS = -0.15  # USD
        self.AUTOBOT_MIN_TARGET_PROFIT = 0.12   # 0.25% profit minimal
    ...
    def _autobot_open_position(self, symbol: str, position: str, mark_price: float, current_candle: datetime):
        ...
        notional = 58.0  # Target notional 100 USDT
    ...
    def _monitor_trailing_stop(self, binance_order_id: str):
        ...
        while not self.shutdown_event.is_set():
            try:
                time.sleep(3)  # Cek setiap 5 detik
                ...
                with self.auto_close_lock:
                    ...
                    max_profit = trail_data['max_profit']  # Dalam USD
            ...
            try:
                ...
                for symbol in self.symbols:
                    ...
                    try:
                        ...
                        if btc_recommendation == 'LONG' and btc_index_val > 0.06:
                            score += 3
                        elif btc_recommendation != 'LONG' and btc_index_val > 0.1:
                            score += 3
                        elif btc_recommendation == 'SHORT' and btc_index_val < -0.06:
                            score -= 3
                        elif btc_recommendation != 'SHORT' and btc_index_val < -0.1:
                            score -= 3

                        # 8. Kekuatan Momentum
                        if momentum_strength > 2.5:
                            score += 1
                        elif momentum_strength < -2.5:
                            score -= 1
                        # ===== LOGIKA SINYAL AKHIR =====
                        signal = 'HOLD'

                        # Aturan utama untuk scalping
                        if score >= 7 and liq_sell_usd > 100:
                            signal = 'LONG'
                        elif score == 6:
                            # Konfirmasi tambahan untuk sinyal kuat
                            if buy_ratio > 85 and liq_sell_usd > 1000:
                                signal = 'LONG'
                        elif score <= -7 and liq_buy_usd > 100:
                            signal = 'SHORT'
                        elif score == -6:
